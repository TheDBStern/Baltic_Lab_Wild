...
# read in data
# either SNP data (e.g. "lab.snps_freq") or haplotype blocks (e.g. "hap_blocks.freq")
genoF <- as.data.table(read.table("data/lab.genobaypass.4chr_freq", h=F, stringsAsFactors=F))
genoF$loc<- 1:nrow(genoF)
genoF_cov <- as.data.table(read.table("data/lab.genobaypass.4chr_cov", h=F, stringsAsFactors=F))
